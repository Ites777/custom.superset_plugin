...
function displayHeaderCell(needToggle, ArrowIcon, onArrowClick, value, namesMapping) {
  ...
  return needToggle ? (0, _react2.jsx)("span", {
  ...
  }, ArrowIcon), (0, _react2.jsx)("span", {
    className: "toggle-val"
  }, parseLabel(name))) : parseLabel(name);
}

class TableRenderer extends _react.default.Component {
  constructor(props) {
    super(props); // We need state to record which entries are collapsed and which aren't.
  ...
  getBasePivotSettings() {
    ...
    const pivotData = new _utilities.PivotData(props, {
    ...
    const colKeys = pivotData.getColKeys(); // Also pre-calculate all the callbacks for cells, etc... This is nice to have to
    ...
    if (tableOptions.clickCallback) {
      rowKeys.forEach(rowKey => {
        const flatRowKey = (0, _utilities.flatKey)(rowKey);
      ...
      if (rowTotals) {
        rowKeys.forEach(rowKey => {
          rowTotalCallbacks[(0, _utilities.flatKey)(rowKey)] = this.clickHandler(pivotData, rowKey, []);
        });
      }
  ...
  collapseAttr(rowOrCol, attrIdx, allKeys) {
    return e => {
      ...
      const collapsed = allKeys.filter(k => k.length === keyLen).map(_utilities.flatKey);
      const updates = {};
      collapsed.forEach(k => {
        updates[k] = true;
      });
      ...
      if (rowOrCol) {
        ...
        this.setState(state => ({
          collapsedCols: { ...state.collapsedCols,
            ...updates
          }
        }));
      }
  ...
  calcAttrSpans(attrArr, numAttrs) {
    ...
    const spans = []; // Index of the last new value
  ...
  renderColHeaderRow(attrName, attrIdx, pivotSettings) {
    ...
    const spaceCell = attrIdx === 0 && rowAttrs.length !== 0 ? (0, _react2.jsx)("th", {
      key: "padding",
    ...
    const attrNameCell = (0, _react2.jsx)("th", {
      key: "label",
      className: "pvtAxisLabel"
    }, displayHeaderCell(needToggle, subArrow, arrowClickHandle, attrName, namesMapping));
    const attrValueCells = [];
    const rowIncrSpan = rowAttrs.length !== 0 ? 1 : 0; // Iterate through columns. Jump over duplicate values.
    ...
    while (i < visibleColKeys.length) {
      ...
      if (attrIdx < colKey.length) {
        ...
        const flatColKey = (0, _utilities.flatKey)(colKey.slice(0, attrIdx + 1));
        ...
        attrValueCells.push((0, _react2.jsx)("th", {
    ...
    const totalCell = attrIdx === 0 && rowTotals ? (0, _react2.jsx)("th", {
      key: "total",
    ...
    }, `Total (${this.props.aggregatorName})`) : null;
  ...
  renderTableRow(rowKey, rowIdx, pivotSettings) {
    ...
    const attrValueCells = rowKey.map((r, i) => {
      ...
      if (rowSpan > 0) {
        const flatRowKey = (0, _utilities.flatKey)(rowKey.slice(0, i + 1));
        ...
        }, displayHeaderCell(needRowToggle, this.state.collapsedRows[flatRowKey] ? arrowCollapsed : arrowExpanded, onArrowClick, headerCellFormattedValue, namesMapping));
      }

      return null;
    });
    const attrValuePaddingCell = rowKey.length < rowAttrs.length ? (0, _react2.jsx)("th", {
    ...
    const valueCells = visibleColKeys.map(colKey => {
      const flatColKey = (0, _utilities.flatKey)(colKey);
    ...
    const rowCells = [...attrValueCells, attrValuePaddingCell, ...valueCells, totalCell];
    return (0, _react2.jsx)("tr", {
  ...
  renderTotalsRow(pivotSettings) {
    ...
    const totalLabelCell = (0, _react2.jsx)("th", {
      key: "label",
    ...
    if (rowTotals) {
      ...
      const aggValue = agg.value();
      grandTotalCell = (0, _react2.jsx)("td", {
        role: "gridcell",
        key: "total",
  ...
  visibleKeys(keys, collapsed, numAttrs, subtotalDisplay) {
    ...
    !key.some((k, j) => collapsed[(0, _utilities.flatKey)(key.slice(0, j))]) && ( // Leaf key.
    key.length === numAttrs || // Children hidden. Must show total.
    (0, _utilities.flatKey)(key) in collapsed || // Don't hide totals.
  ...
  render() {
    ...
    const {
      ...
      rowSubtotalDisplay,
      colSubtotalDisplay
    } = this.cachedBasePivotSettings; // Need to account for exclusions to compute the effective row
    // and column keys.
    ...
    return (0, _react2.jsx)(_Styles.Styles, null, (0, _react2.jsx)("table", {
      className: "pvtTable",
      role: "grid"
    }, (0, _react2.jsx)("thead", null, colAttrs.map((c, j) => this.renderColHeaderRow(c, j, pivotSettings)), rowAttrs.length !== 0 && this.renderRowHeaderRow(pivotSettings)), (0, _react2.jsx)("tbody", null, visibleRowKeys.map((r, i) => this.renderTableRow(r, i, pivotSettings)), colTotals && this.renderTotalsRow(pivotSettings))));
  }
