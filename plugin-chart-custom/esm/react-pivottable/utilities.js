...
const sortAs = function (order) {
  const mapping = {}; // sort lowercased keys similarly
      ...
      return function () {
        return {
          ...
          push(record) {
            ...
            switch (record['Метрика']) {
              case 'Всего запросов':
                this.totalRequests += record[attr];
                break;

              case 'Всего ответов':
                this.totalResponds += record[attr];
                break;

              case '% корректных':
                this.totalResponds += record['Всего ответов'];
                this.correct += record['Из них корректных'];
                break;

              case '% корректных ответов в срок':
                this.pctCorrectResponsesOnTime.sum += record[attr];
                this.pctCorrectResponsesOnTime.count += 1;
                break;

              case '% ответов в срок':
                this.pctResponsesOnTime.sum += record[attr];
                this.pctResponsesOnTime.count += 1;
                break;

              case '% ответов на запросы':
                this.totalResponds += record['Всего запросов'];
                this.totalRequests += record['Всего ответов'];
                break;

              case 'Из них корректных':
                this.correct += record[attr];
                break;

              case 'Из них срок прошёл':
                this.deadlineHasPassed += record[attr];
                break;

              case 'Ответы не получены':
                this.noResponses += record[attr];
                break;

              default:
                this.totalResponds += 0;
                break;
            }
          },

          value(x) {
            switch (this.record['Метрика'] || x) {
              case 'Всего запросов':
                return this.totalRequests;

              case 'Всего ответов':
                return this.totalResponds;

              case '% корректных':
                return this.correct / this.totalResponds * 100;

              case '% корректных ответов в срок':
                return this.pctCorrectResponsesOnTime.sum / this.pctCorrectResponsesOnTime.count;

              case '% ответов в срок':
                return this.pctResponsesOnTime.sum / this.pctResponsesOnTime.count;

              case '% ответов на запросы':
                return this.totalResponds / this.totalRequests * 100;
...
const aggregatorTemplates = { ...baseAggregatorTemplates,
  ...extendedAggregatorTemplates
}; // default aggregators & renderers use US naming and number formatting

const aggregators = (tpl => ({
  Custom: tpl.custom(usFmtInt),
...
class PivotData {
  constructor(inputProps = {}, subtotals = {}) {
    this.props = { ...PivotData.defaultProps,
      ...inputProps
    };
  ...
  getCustomAggregator() {
    const custom = 'Custom';
    return this.props.aggregatorsFactory(this.props.defaultFormatter)[custom](this.props.vals);
  }
  ...
  processRecord(record) {
    ...
    for (let ci = colStart; ci <= colKey.length; ci += 1) {
      ...
      if (!this.colTotals[flatColKey]) {
        ...
        this.colTotals[flatColKey] = this.getCustomAggregator(record, colKey)(this, [], fColKey);
      }
    ...
    for (let ri = rowStart; ri <= rowKey.length; ri += 1) {
      ...
      for (let ci = colStart; ci <= colKey.length; ci += 1) {
        ...
        if (!this.tree[flatRowKey][flatColKey]) {
          if (isRowSubtotal) {
            this.tree[flatRowKey][flatColKey] = this.getCustomAggregator(record)(this, fRowKey, fColKey);
          } else {
...
PivotData.propTypes = {
  ...
  rowOrder: PropTypes.oneOf(['key_a_to_z', 'key_z_to_a', 'value_a_to_z', 'value_z_to_a']),
  colOrder: PropTypes.oneOf(['key_a_to_z', 'key_z_to_a', 'value_a_to_z', 'value_z_to_a'])
};
export { aggregatorTemplates, aggregators, derivers, locales, naturalSort, numberFormat, getSort, sortAs, flatKey, PivotData };