...
function displayHeaderCell(needToggle, ArrowIcon, onArrowClick, value, namesMapping) {
  ...
  return needToggle ? ___EmotionJSX("span", {
...
export class TableRenderer extends React.Component {
    ...
    if (tableOptions.clickCallback) {
      rowKeys.forEach(rowKey => {
        ...
        colKeys.forEach(colKey => {
          cellCallbacks[flatRowKey][flatKey(colKey)] = this.clickHandler(pivotData, rowKey, colKey);
        });
      }); // Add in totals as well.

      if (rowTotals) {
        rowKeys.forEach(rowKey => {
          rowTotalCallbacks[flatKey(rowKey)] = this.clickHandler(pivotData, rowKey, []);
        });
      }

      if (colTotals) {
        colKeys.forEach(colKey => {
          colTotalCallbacks[flatKey(colKey)] = this.clickHandler(pivotData, [], colKey);
        });
      }
  ...
  renderColHeaderRow(attrName, attrIdx, pivotSettings) {
    ...
    const spaceCell = attrIdx === 0 && rowAttrs.length !== 0 ? ___EmotionJSX("th", {
      key: "padding",
    ...
    const attrNameCell = ___EmotionJSX("th", {
      key: "label",
    ...
    while (i < visibleColKeys.length) {
      ...
      if (attrIdx < colKey.length) {
        ...
        attrValueCells.push(___EmotionJSX("th", {
    ...
    const totalCell = attrIdx === 0 && rowTotals ? ___EmotionJSX("th", {
      key: "total",
  ...
  renderTableRow(rowKey, rowIdx, pivotSettings) {
    ...
    const attrValuePaddingCell = rowKey.length < rowAttrs.length ? ___EmotionJSX("th", {
  ...
  renderTotalsRow(pivotSettings) {
    ...
    const totalLabelCell = ___EmotionJSX("th", {
      key: "label",
    ...
    if (rowTotals) {
      ...
      const aggValue = agg.value();
      grandTotalCell = ___EmotionJSX("td", {
        role: "gridcell",
        key: "total",
  ...
  visibleKeys(keys, collapsed, numAttrs, subtotalDisplay) {
    ...
    !key.some((k, j) => collapsed[flatKey(key.slice(0, j))]) && ( // Leaf key.
    key.length === numAttrs || // Children hidden. Must show total.
    flatKey(key) in collapsed || // Don't hide totals.
  ...
  render() {
    ...
    return ___EmotionJSX(Styles, null, ___EmotionJSX("table", {
      className: "pvtTable",
      role: "grid"
    }, ___EmotionJSX("thead", null, colAttrs.map((c, j) => this.renderColHeaderRow(c, j, pivotSettings)), rowAttrs.length !== 0 && this.renderRowHeaderRow(pivotSettings)), ___EmotionJSX("tbody", null, visibleRowKeys.map((r, i) => this.renderTableRow(r, i, pivotSettings)), colTotals && this.renderTotalsRow(pivotSettings))));
  }
