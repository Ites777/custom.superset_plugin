...
import { styled, getNumberFormatter, useTheme } from '@superset-ui/core'; // @ts-ignore
...
const aggregatorsFactory = formatter => ({
  Custom: aggregatorTemplates.custom(formatter),
...
export default function PivotTableChart(props) {
  ...
  const metricFormatters = hasCustomMetricFormatters && Object.fromEntries(columnFormatsArray.map(([metric, format]) => [metric, getNumberFormatter(format)]));
  const metricNames = useMemo(() => metrics.map(metric => typeof metric === 'string' ? metric : metric.label), [metrics]);
  const unpivotedData = useMemo(() => data.reduce((acc, record) => [...acc, ...metricNames.map(name => ({ ...record,
    [METRIC_KEY]: name,
    value: record[name]
  ...
  return ___EmotionJSX(Styles, {
  ...
  }, ___EmotionJSX(PivotTableWrapper, null, ___EmotionJSX(PivotTable, {
    data: unpivotedData,
    rows: rows,
    cols: cols,
    ...
    subtotalOptions: {
      ...
      arrowCollapsed: ___EmotionJSX(PlusSquareOutlined, {
        style: iconStyle
      }),
      arrowExpanded: ___EmotionJSX(MinusSquareOutlined, {
        style: iconStyle
      })
    },
